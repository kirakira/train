...
void arcLengthCB(Fl_Widget*, TrainWindow* tw)
{
...
void physicsCB(Fl_Widget*, TrainWindow* tw)
{
	if (tw->physics->value() == 1)
		tw->arcLength->value(1);
	tw->damageMe(true);
}

void tensionCB(Fl_Widget*, TrainWindow* tw)
{
	tw->world.tension = tw->tension->value();
	tw->damageMe(true);
}

void adaptiveCB(Fl_Widget*, TrainWindow* tw)
{
	tw->damageMe(false);
}
